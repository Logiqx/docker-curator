...
WORKDIR /home/jovyan/work/docker-curator
COPY --chown=jovyan:users python/*.*py* ./python/
COPY --chown=jovyan:users config/*.json ./config/
...
USER ${PY_USER}
WORKDIR /home/${PY_USER}/work/docker-curator
COPY --from=builder --chown=jovyan:jovyan /home/jovyan/work/docker-curator/ ./
RUN mkdir docs
